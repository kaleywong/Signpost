...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int row = 0; row < _width; row++) {
            for (int col = 0; col < _height; col++) {
                if (_solution[row][col] == 1) {
                    _board[row][col] = new Sq(row, col, 1, true, 0, 0);
                } else if (_solution[row][col] == last) {
                    _board[row][col] = new Sq(row, col, last, true, 0, 0);
                } else {
                    _board[row][col] = new Sq(row, col, 0, false, 0, -1);
                }
                _allSquares.add(_board[row][col]);
                _solnNumToPlace[_solution[row][col] - 1] = Place.pl(row, col);
            }
        }
        for (Place pl : _solnNumToPlace) {
            if (pl == null) {
                throw badArgs("Solution must have all diff # from 1 to last.");
            }
        }
        for (Sq square : _allSquares) {
            square._dir = arrowDirection(square.x, square.y);
        }
        for (int row = 0; row < _width; row++) {
            for (int col = 0; col < _height; col++) {
                _board[row][col]._successors
                        = _allSuccessors[row][col][_board[row][col]._dir];
            }
        }
        for (Sq square : _allSquares) {
        ...
        for (Sq sq : _allSquares) {
            for (Place sp : allSuccessors(sq.pl, sq._dir)) {
                get(sp)._predecessors.add(sq.pl);
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        _board = new Sq [_width][_height];
        for (int a = 0; a < _width; a += 1) {
            for (int b = 0; b < _height; b += 1) {
        ...
        for (int a = 0; a < _width; a += 1) {
            for (int b = 0; b < _height; b += 1) {
                Sq x = _board[a][b];
                Sq y = model._board[a][b];
                x._successor = y._successor;
                x._predecessor = y._predecessor;
                x._head = y._head;
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq sq : this) {
            for (Sq connectto : this) {
                if (connectto.sequenceNum() != 0
                        && sq.sequenceNum() != 0) {
                    if (sq.connectable(connectto)) {
                        sq.connect(connectto);
                        result = true;
                    }
                    if (connectto.connectable(sq)) {
                        connectto.connect(sq);
                        result = true;
                    }
                }
            }
    ...
    void solve() {
        for (int row = 0; row < _width; row++) {
            for (int column = 0; column < _height; column++) {
                if (!_board[row][column]._hasFixedNum) {
                    _board[row][column]._sequenceNum = _solution[row][column];
                }
            }
        }
        for (int i = 1; i < this.size(); i += 1) {
            ...
            for (Sq sq : this) {
                ...
                if (sq.sequenceNum() == i + 1) {
                    second = sq;
                }
            }
            first._successor = second;
            second._predecessor = first;
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (_solnNumToPlace.length > seq0) {
            Place mid = _solnNumToPlace[seq0];
            return (Place.dirOf(x, y, mid.x, mid.y));
        } else {
            return 0;
        }
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (s1._sequenceNum == 1 || this.sequenceNum()
                    == _width * _height) {
                return false;
            }
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0 && s1._sequenceNum == 0) {
                for (Sq p = this._successor; p != null; p = p._successor) {
                    p._sequenceNum = p._predecessor._sequenceNum + 1;
                }
                releaseGroup(sgroup);
            }
            if (s1._sequenceNum != 0 && this._sequenceNum == 0) {
                for (Sq k = s1._predecessor; k != null; k = k._predecessor) {
                    k._sequenceNum = k._successor._sequenceNum - 1;
                }
            ...
            Sq heads = this;
            while (heads._successor != null) {
                heads._successor._head = this._head;
                heads = heads._successor;
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                for (Sq a = this; a != null; a = a._predecessor) {
                    if (a._hasFixedNum) {
                        fixedthis = true;
                    }
                }
                if (this._predecessor == null && !this._hasFixedNum) {
                    this._sequenceNum = 0; this._head._group = -1;
                }
                if (!fixedthis) {
                    if (this._predecessor != null) {
                        this._head._group = newGroup();
                        for (Sq b = this; b != null; b = b._predecessor) {
                            b._sequenceNum = 0;
                        }
                    }
                }
                boolean fixednext = false;
                for (Sq a = next; a != null; a = a._successor) {
                    if (a._hasFixedNum) {
                        fixednext = true;
                    }
                }
                if (next._successor == null && !next._hasFixedNum) {
                    next._head._group = -1; next._sequenceNum = 0;
                }
                if (!fixednext && !next._hasFixedNum) {
                    if (next._successor != null) {
                        next._sequenceNum = 0; next._group = newGroup();
                        for (Sq b = next;
                             b._successor != null; b = b._successor) {
                            b._successor._sequenceNum = 0;
                        }
                    }
                }
            }
            next._head = next; Sq fixedhead = next;
            while (fixedhead._successor != null) {
                fixedhead._successor._head = next;
                fixedhead = fixedhead._successor;
            }
        }
